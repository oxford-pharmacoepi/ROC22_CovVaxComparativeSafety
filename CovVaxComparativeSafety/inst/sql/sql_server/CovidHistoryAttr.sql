...
CREATE TABLE #covid_cohort_table (
...
INSERT INTO #covid_cohort_table (cohort_definition_id, subject_id, cohort_start_date, cohort_end_date)
...
/* covid history of vaccinated people */
...
INSERT INTO #covid_history (cohort_definition_id, subject_id, cohort_start_date, cohort_end_date, COVID_DATE, days_covid_vax )
SELECT *, DATEDIFF(day, COVID_DATE,cohort_start_date ) as days_covid_vax 
from(
      SELECT VAX.cohort_definition_id, VAX.subject_id, VAX.cohort_start_date, VAX.cohort_END_date, MIN(COVID.COHORT_START_DATE) AS COVID_DATE  
      FROM @cohort_database_schema.@study_cohort_table VAX 
      LEFT JOIN #covid_cohort_table COVID 
      ON VAX.SUBJECT_ID = COVID.SUBJECT_ID 
      AND COVID.COHORT_START_DATE  < VAX.COHORT_START_DATE 
      WHERE VAX.cohort_definition_id in (@VAX_definition_ids)
      GROUP BY VAX.cohort_definition_id, VAX.subject_id, VAX.cohort_start_date, VAX.cohort_END_date
      ) foo 
;


/* cohort_attribute_table */
...
CASE WHEN days_covid_vax >0 then 1 else 0 end AS value_as_number
INTO @cohort_database_schema.@cohort_attribute_table
FROM #covid_history covhis
...
'Covid-19 infection prior vaccination' AS attribute_name
INTO @cohort_database_schema.@attribute_definition_table;
	

TRUNCATE TABLE #covid_cohort_table;
DROP TABLE #covid_cohort_table;

TRUNCATE TABLE #covid_history;
