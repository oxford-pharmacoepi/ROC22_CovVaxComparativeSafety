...
(
  select de.* 
  FROM @cdm_database_schema.DRUG_EXPOSURE de
JOIN #Codesets cns on (de.drug_source_concept_id = cns.concept_id and cns.codeset_id = 1)
) C
...
(
  select o.* 
  FROM @cdm_database_schema.OBSERVATION o
JOIN #Codesets cns on (o.observation_source_concept_id = cns.concept_id and cns.codeset_id = 1)
) C
