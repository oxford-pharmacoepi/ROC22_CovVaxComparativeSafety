...
exportAnalyses <- function(outputFolder, exportFolder) {
  ...
  cmAnalysisListFile <- system.file("settings",
                                    "cmAnalysisList.json",
                                    package = "CovVaxComparativeSafety")
...
exportExposures <- function(outputFolder, exportFolder) {
  ...
  pathToCsv <- system.file("settings", "TcosOfInterest.csv", package = "CovVaxComparativeSafety")
  ...
  createExposureRow <- function(exposureId) {
    ...
    cohortFileName <- system.file("cohorts", paste0(name, ".json"), package = "CovVaxComparativeSafety")
...
exportOutcomes <- function(outputFolder, exportFolder) {
  ...
  pathToCsv <- system.file("settings", "NegativeControls.csv", package = "CovVaxComparativeSafety")
...
exportDiagnostics <- function(outputFolder,
  ...
  cluster <- ParallelLogger::makeCluster(min(4, maxCores))
  ParallelLogger::clusterRequire(cluster, "CovVaxComparativeSafety")
...
exportIncidenceRate <- function(outputFolder, exportFolder,databaseId){
  ParallelLogger::logInfo("- 7,14,28 days crude incidence rate table")
  
  outcomesOfInterest <- getOutcomesOfInterest()
  reference <- readRDS(file.path(outputFolder, "cmOutput", "outcomeModelReference.rds"))
  reference <- reference[reference$outcomeId %in% outcomesOfInterest, ]
  IRcrudeResults <- data.frame()
  
  for (i in 1:nrow(reference)) {  # i=2
    studyPop <- readRDS(file.path(outputFolder,
                                  "cmOutput",
                                  reference$studyPopFile[i]))
    
    IRcrude <- studyPop %>% 
      mutate(survivalTime_7d = if_else(survivalTime>=7 ,7, survivalTime),
             survivalTime_14d = if_else(survivalTime>=14 ,14, survivalTime),
             survivalTime_21d = if_else(survivalTime>=21 ,21, survivalTime)) %>%
      group_by(treatment) %>%
      summarise(Patients = n(),
                # 7 day
                PersonYears_7d = sum(survivalTime_7d) / 365.25 , 
                Events_7d = sum(ifelse(outcomeCount > 0 & 7 >= daysToEvent , 1, 0)),  
                ir1k_7d = round(1000 * Events_7d / PersonYears_7d, 3),
                ip1k_7d = round(1000 * Events_7d / Patients, 3),
                # 14 day
                PersonYears_14d = sum(survivalTime_14d) / 365.25 , 
                Events_14d = sum(ifelse(outcomeCount > 0 & 14 >= daysToEvent , 1, 0)),  
                ir1k_14d = round(1000 * Events_14d / PersonYears_14d, 3),
                ip1k_14d = round(1000 * Events_14d / Patients, 3),
                # 21 day
                PersonYears_21d = sum(survivalTime_21d) / 365.25 , 
                Events_21d = sum(ifelse(outcomeCount > 0 & 21 >= daysToEvent , 1, 0)),  
                ir1k_21d = round(1000 * Events_21d / PersonYears_21d, 3),
                ip1k_21d = round(1000 * Events_21d / Patients, 3),
                # 28 day
                PersonYears = sum(survivalTime) / 365.25 , 
                Events = sum(ifelse(outcomeCount > 0, 1, 0)),  
                ir1k = round(1000 * Events / PersonYears, 3),
                ip1k = round(1000 * Events / Patients, 3)
      )
    IRcrude$analysisId <- reference$analysisId[i]
    IRcrude$exposureId <- reference$targetId[i]
    IRcrude$comparatorId <- reference$comparatorId[i]
    IRcrude$outcomeId <- reference$outcomeId[i]
    IRcrude$databaseId <- databaseId
    
    IRcrudeResults <- rbind(IRcrudeResults, IRcrude)
  }
  
  fileName <- file.path(exportFolder, "incidence_rate_crude.csv")
  readr::write_csv(IRcrudeResults, fileName)
  rm(IRcrudeResults) 
  
  
  
  ParallelLogger::logInfo("- 7,14,28 days matched incidence rate table")
  
  outcomesOfInterest <- getOutcomesOfInterest()
  reference <- readRDS(file.path(outputFolder, "cmOutput", "outcomeModelReference.rds"))
  reference <- reference[reference$outcomeId %in% outcomesOfInterest, ]
  reference <- reference[reference$strataFile != "",]
  IRmatchedeResults <- data.frame()
  ...
  for (i in 1:nrow(reference)) {  # i=2
    ...
    IRmatched$analysisId <- reference$analysisId[i]
    IRmatched$exposureId <- reference$targetId[i]
    IRmatched$comparatorId <- reference$comparatorId[i]
    IRmatched$outcomeId <- reference$outcomeId[i]
    IRmatched$databaseId <- databaseId
    
    IRmatchedeResults <- rbind(IRmatchedeResults, IRmatched)
  }
  
  fileName <- file.path(exportFolder, "incidence_rate_matched.csv")
  readr::write_csv(IRmatchedeResults, fileName)
  rm(IRmatchedeResults) 
}

exportAnalysisSummary <-  function(outputFolder, exportFolder){
  ParallelLogger::logInfo("Add AnalysisSummary.csv table ")
  
  files2 <- list.files(outputFolder, pattern = ".*\\.csv$")    ###XL
  files2.raw <- paste(outputFolder, files2, sep = .Platform$file.sep)   ####XL
  file.copy(files2.raw, exportFolder)
}